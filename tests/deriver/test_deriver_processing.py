...
class TestDeriverProcessing:
    ...
    async def test_all_observer_saves_failing_surfaces_failure(self):
        ...
        mock_response = HonchoLLMCallResponse(
            content=ExtractedRepresentation(
                explicit=[
                    ExtractedObservation(content="The user has a dog named Rover")
                ]
            ),
            input_tokens=10,
            output_tokens=5,
        ...
        admission_response = HonchoLLMCallResponse(
            content=AdmissionRepresentation(
                explicit=[
                    AdmissionDecision(
                        content="The user has a dog named Rover",
                        action="create",
                        reason_for_entry="Durable fact about the user",
                        admission_case_id=0,
        ...
        with (
            patch(
                "src.deriver.deriver.honcho_llm_call",
                new_callable=AsyncMock,
                side_effect=[mock_response, admission_response],
            ),
    ...
    async def test_retryable_observer_save_reraises_after_telemetry(self):
        ...
        partial_save = AsyncMock(side_effect=[1, deadlock])
        emitted: list[Any] = []
        with (
            patch(
                "src.deriver.deriver.honcho_llm_call",
