...
falls back to an empty ``ExtractedRepresentation`` and the run looks like "the
...
def test_deriver_response_model_round_trips() -> None:
    ...
    schema = ExtractedRepresentation.model_json_schema()
    content = json.dumps(generate(schema))

    representation = ExtractedRepresentation.model_validate_json(content)
    assert (
